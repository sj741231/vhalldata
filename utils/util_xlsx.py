...
class HandleXLSX(object):
    ...
    def generate_row_object_iterator(self, check_file, sheet_name=None, start_point=None, end_point=None, **kwargs):
        ...
        for row_number, row in enumerate(_sheet_iter_rows, start=_start_point):
            row_value_list = [self.get_format_value(cell.value, **kwargs) for cell in row]
            if row_value_list:
    ...
    def get_format_value(self, value, **kwargs):
        """
        Format value
        ...
        if isinstance(value, float):
            ...
            _value = value
        return self.modify_value(_value, **kwargs)

    def modify_value(self, value, **kwargs):
        """
        custom modify value
        :param value:
        :param kwargs:
    ...
    def get_column_name_list(self, sheet_name=None, **kwargs):
        ...
        return [str(column_name).strip() for column_name in _column_name_list]
...
if __name__ == "__main__":
    ...
    print("***" * 30)
    rows = xls.generate_row_object_iterator(check_file=False, sheet_name='listing', start_point=2, end_point=5)
    ...
    xls1 = HandleXLSX(filename)

    print("***" * 30)
    rows1 = xls1.generate_row_object_iterator(check_file=False, sheet_name='dict', start_point=2, end_point=5)
